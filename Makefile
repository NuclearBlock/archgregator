...
LD_FLAGS = -X github.com/nuclearblock/archgregator/cmd.Version=$(VERSION) \
	-X github.com/nuclearblock/archgregator/cmd.Commit=$(COMMIT)
...
build: go.sum
ifeq ($(OS),Windows_NT)
	@echo "building archgregator binary..."
	@go build -mod=readonly $(BUILD_FLAGS) -o build/archgregator.exe ./cmd/archgregator
else
	@echo "building archgregator binary..."
	@go build -mod=readonly $(BUILD_FLAGS) -o build/archgregator ./cmd/archgregator
endif
.PHONY: build
...
install: go.sum
	@echo "installing archgregator binary..."
	@go install -mod=readonly $(BUILD_FLAGS) ./cmd/archgregator
.PHONY: install
