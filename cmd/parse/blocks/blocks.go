...
func newAllCmd(parseConfig *parsecmdtypes.Config) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "all",
		Short: "Fix missing blocks, messages and events in database",
...
`, flagStart, flagEnd, flagForce),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			workerCtx := parser.NewContext(parseCtx.EncodingConfig, parseCtx.Node, parseCtx.Database, parseCtx.Logger)
